...
const LogisticsPage = () => {
            ...
            setReadyParts(
                data?.map((item: any): Part => ({
                    ...
                    vehicle: Array.isArray(item.vehicle)
                        ? {
                            ...item.vehicle[0],
                            user_profile: Array.isArray(item.vehicle[0]?.user_profile)
                                ? item.vehicle[0].user_profile[0]
                                : item.vehicle[0]?.user_profile,
                        }
                        : item.vehicle,
                    bids: item.bids?.map((bid: any) => ({
                        id: bid.id,
                        status: bid.status,
                        is_sourcer_provided: bid.is_sourcer_provided || false,
                        vendor_info: bid.vendor_info,
                        vendor: Array.isArray(bid.vendor)
                            ? bid.vendor[0]
                            : bid.vendor
                    })) || []
                })) || []
            ...
            setCompletedInvoices(
                data?.map((item) => ({
                    ...
                    user_profile: Array.isArray(item.user_profile)
                        ...
                        : item.user_profile,
                    invoice_parts:
                        item.invoice_parts?.map((invoicePart) => ({
                            ...invoicePart,
                            part: Array.isArray(invoicePart.part)
                                ? invoicePart.part[0]
                                : invoicePart.part,
                        })) || [],
                })) || []
    ...
    const readyPartsByLocation = readyParts.reduce((acc, part) => {
        const bid = part.bids[0];
        if (!bid) return acc;
        ...
        if (bid.is_sourcer_provided && bid.vendor_info) {
            ...
            vendorDetails = {
                full_name: bid.vendor_info.name,
                business_name: bid.vendor_info.business_name,
                location: bid.vendor_info.address,
                is_sourcer_provided: true,
            };
            locationKey = `${bid.vendor_info.name} - ${bid.vendor_info.address} (Sourcer Added)`;
        } else {
            // Handle regular vendor info
            vendorDetails = bid.vendor;
            locationKey = `${bid.vendor.business_name || bid.vendor.full_name} - ${bid.vendor.location}`;
        }
    ...
    const activeDeliveriesByBuyer = activeParts.reduce((acc, part) => {
            ...
            {activeTab === "collection" && (
                <div>
                    ...
                    {readyParts.length === 0 ? (
                        ...
                        Object.entries(readyPartsByLocation).map(
                            ([location, { vendor, parts }]) => (
                                <div key={location} className="mb-6">
                                    <div className="bg-gray-100 px-4 py-2 font-semibold text-gray-700 rounded-t flex justify-between items-center">
                                        <div>
                                            Vendor:{" "}
                    ...
                    <div className="bg-white rounded-lg shadow overflow-x-auto">
                        {activeParts.length === 0 ? (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {Object.entries(
                                        activeDeliveriesByBuyer
                                    ).map(([buyerName, { buyer, parts }]) => {
                                        const driver = mockDrivers[0]; // Mock driver assignment
                                        const isExpanded =
                                            expandedDelivery === buyerName;

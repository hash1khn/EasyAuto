...
interface AcceptedQuote {
    ...
    price: number;
    customerPaid: number; // ADDED
    paidToVendor: number; // ADDED
    profit: number; // ADDED
...
const QuoteHistory: React.FC = () => {
    ...
    const [selectedVendor, setSelectedVendor] = useState<VendorInfo | null>(
        null
    );
    const [isInspectionModalOpen, setIsInspectionModalOpen] = useState(false);
    const [selectedQuoteForInspection, setSelectedQuoteForInspection] =
        useState<AcceptedQuote | null>(null);
    ...
    const fetchAcceptedQuotes = async () => {
        ...
        `
            ...
            const transformedQuotes: AcceptedQuote[] = (data as any[]).map(
                (bid) => {
                    // Extract first element from arrays returned by Supabase for relations
                    const part = Array.isArray(bid.parts)
                        ? bid.parts[0]
                        : bid.parts;
                    const vehicle =
                        part && Array.isArray(part.vehicles)
                            ? part.vehicles[0]
                            : part?.vehicles;
                    const order =
                        part && Array.isArray(part.orders)
                            ? part.orders[0]
                            : part?.orders;
                    const userProfile =
                        order && Array.isArray(order.user_profiles)
                            ? order.user_profiles[0]
                            : order?.user_profiles;
                    const isSourcerProvided = bid.is_sourcer_provided;
                    const vendorInfo = bid.vendor_info || {};
                    const vendor = Array.isArray(bid.vendor)
                        ? bid.vendor[0]
                        : bid.vendor;
                    ...
                    const profitMargin =
                        paidToVendor > 0 ? (profit / paidToVendor) * 100 : 0;
                    ...
                    return {
                        ...
                        price: bid.price,
                        vendor: {
                            name: isSourcerProvided
                                ? vendorInfo.name
                                : vendor?.full_name || "Unknown Vendor",
                            phone: isSourcerProvided
                                ? vendorInfo.phone
                                : vendor?.whatsapp_number || "N/A",
                            email: isSourcerProvided
                                ? vendorInfo.email
                                : vendor?.user?.email || "N/A",
                            address: isSourcerProvided
                                ? vendorInfo.address
                                : vendor?.location || "N/A",
                            isSourcerProvided,
                        },
            ...
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">
                        You need to be signed in to view your quote history.
                    </p>
                </div>
            </div>
        );
                ...
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">
                            View all your accepted quotes and track their
                            delivery status.
                        </p>
                    </div>
                ...
                <div className="text-center text-gray-500 py-12">
                    ...
                    {acceptedQuotes.length === 0 && (
                        <p className="text-sm mt-2">
                            Start accepting vendor quotes to see them here.
                        </p>
                    )}
                </div>
            ) : (
                        ...
                        <div className="p-6 border-b flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    for {selectedQuoteForInspection.partName}
                                </p>
                            </div>
                            <Button
                                variant="ghost"
                        ...
                        <div className="p-6 flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                ...
                                <Carousel className="w-full">
                                    <CarouselContent>
                                        {selectedQuoteForInspection.inspection.images.map(
                                            (img, idx) => (
                                    ...
                                    {selectedQuoteForInspection.inspection
                                        .images.length > 1 && (
                                        <>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mt-2">
                                        ...
                                        <div className="font-semibold text-blue-600">
                                            AED{" "}
                                            {selectedQuoteForInspection.customerPaid.toFixed(
                                                2
                                            )}
                                        </div>
                                        ...
                                        <div className="font-semibold text-gray-700">
                                            AED{" "}
                                            {selectedQuoteForInspection.paidToVendor.toFixed(
                                                2
                                            )}
                                        </div>
                                        ...
                                        <div
                                            className={`font-semibold ${
                                                selectedQuoteForInspection.profit >=
                                                0
                                                    ? "text-green-600"
                                                    : "text-red-600"
                                            }`}>
                                            AED{" "}
                                            {selectedQuoteForInspection.profit.toFixed(
                                                2
                                            )}
                                        </div>
                                        ...
                                        <div
                                            className={`font-semibold ${
                                                selectedQuoteForInspection.profitMargin <
                                                15
                                                    ? "text-red-600"
                                                    : selectedQuoteForInspection.profitMargin ===
                                                      15
                                            ...
                                            {selectedQuoteForInspection.profitMargin.toFixed(
                                                2
                                            )}
                                            %
                                        </div>
                                ...
                                <div>
                                    ...
                                    <div className="bg-gray-50 p-3 rounded-md border">
                                        ...
                                        {selectedQuoteForInspection.vendor
                                            .isSourcerProvided && (
                                            <Badge
                                                variant="secondary"
                                                className="mt-1">

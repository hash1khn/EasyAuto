...
import { calculateProfitMargin } from "@/utils/sourcerUtils"; // Add this import
...
export const AdminOrders = () => {
    const [activeSubTab, setActiveSubTab] = useState<"open" | "history">(
        "open"
    );
    ...
    const [receiptInvoiceId, setReceiptInvoiceId] = useState<string | null>(
        null
    );
            ...
            const transformedRefundRequests: RefundRequest[] =
                refundRequestsData?.map((req) => ({
                    ...
                    user: {
                        ...
                        whatsapp_number:
                            req.user_profiles?.whatsapp_number || "",
    ...
    const fetchOrders = async () => {
        ...
        `
            ...
            const { data: pendingRefunds, error: pendingRefundsError } =
                await supabase
                    .from("refund_requests")
                    .select("part_id")
                    .in("part_id", partIds)
            ...
            const partsWithPendingRefunds = new Set(
                pendingRefunds?.map((r) => r.part_id) || []
            );
        ...
        `
            ...
            const userIds =
                ordersData
                    ?.map((order) => order.user_profiles?.id)
                    .filter(Boolean) || [];
        ...
        `
            ...
            const { data: invoicePartsData, error: invoicePartsError } =
                await supabase
                    .from("invoice_parts")
                    .select(
                        `
                ...
                ordersData?.map((order) => {
                    ...
                    orderParts.forEach((part) => {
                        if (!vehicleMap.has(part.vehicle_id)) {
                            ...
                            const vehicleData = Array.isArray(part.vehicles)
                                ? part.vehicles[0]
                                : part.vehicles;
                        ...
                        const acceptedBid = bidsData?.find(
                            (bid) => bid.part_id === part.id
                        );
                        const vendorProfile = Array.isArray(
                        ...
                        if (acceptedBid) {
                            totalCustomerPayment +=
                                (acceptedBid.customer_paid || 0) *
                                (part.quantity || 1);
                            totalVendorPayment +=
                                (acceptedBid.price || 0) * (part.quantity || 1);
                        }
                        ...
                        vehicleMap.get(part.vehicle_id).parts.push({
                            ...
                            has_pending_refund: partsWithPendingRefunds.has(
                                part.id
                            ),
                    ...
                    const totalProfit =
                        totalCustomerPayment - totalVendorPayment;
                    ...
                    const userData = Array.isArray(userProfile?.users)
                        ? userProfile?.users[0]
                        : userProfile?.users;
                    ...
                    const userInvoices =
                        invoicesData?.filter(
                            (inv) => inv.user_id === userProfile?.id
                        ) || [];

                    // Filter deliveries to only include those with parts from this specific order
                    const relevantDeliveries = userInvoices.filter(
                        (invoice) => {
                            const invoiceParts =
                                invoicePartsData?.filter(
                                    (ip) => ip.invoice_id === invoice.id
                                ) || [];
                            // Check if this invoice contains any parts from the current order
                            return invoiceParts.some((ip) =>
                                orderPartIds.includes(ip.part_id)
                            );
                        }
                    );
                    ...
                    const deliveries = relevantDeliveries.map((invoice) => {
                        ...
                        const orderRelevantInvoiceParts = invoiceParts.filter(
                            (ip) => orderPartIds.includes(ip.part_id)
                        );

                        const partsBreakdown = orderRelevantInvoiceParts.map(
                            (ip) => {
                                const part = partsData?.find(
                                    (p) => p.id === ip.part_id
                                );
                                const bid = bidsData?.find(
                                    (b) => b.part_id === ip.part_id
                                );
                                const bidVendorProfile = Array.isArray(
                                    bid?.user_profiles
                                )
                                    ? bid?.user_profiles[0]
                                    : bid?.user_profiles;
                                return {
                                    ...
                                    vendor_name:
                                        bidVendorProfile?.business_name ||
                                        bidVendorProfile?.full_name ||
                                        "N/A",
                                };
                            }
                        );
                        ...
                        vehicles.forEach((vehicle) => {
                            vehicle.parts.forEach((part) => {
                                if (part.accepted_bid) {
                                    totalCustomerPayment +=
                                        part.accepted_bid.customer_paid *
                                        part.quantity;
                                    totalVendorPayment +=
                                        part.accepted_bid.price * part.quantity;
                                }
                            });
                        });
                        ...
                        return {
                            ...
                            parts: orderRelevantInvoiceParts.map(
                                (ip) => ip.part_id
                            ),
                            invoice: {
                                id: invoice.id,
    ...
    const confirmDeletePart = async () => {
        ...
        try {
            ...
            if (remainingParts && remainingParts.length === 0) {
                ...
                alert(
                    "Deleted the last part in the order, so the entire order has been deleted."
                );
            } else {
    ...
    const handleApproveRefund = async () => {
        ...
        try {
            ...
            if (selectedRefundRequest.invoice_id) {
                ...
                const { data: allInvoiceParts, error: allPartsError } =
                    await supabase
                        .from("invoice_parts")
                        .select("part_id")
                ...
                const { data: refundedParts, error: refundedPartsError } =
                    await supabase
                        .from("parts")
                        .select("id")
                        .in(
    ...
    return (
        <div className="space-y-6">
            <Tabs defaultValue="orders" className="w-full">
                ...
                <TabsContent value="orders" className="space-y-6">
                    <div className="flex items-center gap-4">
                        <Button
                            variant={
                                activeSubTab === "open" ? "default" : "outline"
                            }
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {filteredOrders.map((order) => {
                                    ...
                                    const totalParts = order.vehicles.flatMap(
                                        (v) => v.parts
                                    ).length;
                                    const hasPendingRefunds = order.vehicles
                                        .flatMap((v) => v.parts)
                                        .some((p) => p.has_pending_refund);
                                        ...
                                        <TableRow key={order.id}>
                                            ...
                                            {activeSubTab === "history" && (
                                                <>
                                                    <TableCell>
                                                        AED{" "}
                                                        {order.totalCustomerPayment.toFixed(
                                                            2
                                                        )}
                                                    </TableCell>
                                                    <TableCell>
                                                        AED{" "}
                                                        {order.totalVendorPayment.toFixed(
                                                            2
                                                        )}
                                                    </TableCell>
                                                    <TableCell>
                            ...
                            <TableBody>
                                {refundRequests.map((request) => (
                                    <TableRow key={request.id}>
                                        ...
                                        <TableCell>
                                            {request.part.vehicle.make}{" "}
                                            {request.part.vehicle.model}{" "}
                                            {request.part.vehicle.year}
                                        </TableCell>
                    ...
                    <DialogContent className="max-w-3xl p-0 bg-transparent shadow-none">
                        <Card className="w-full max-h-[90vh] overflow-y-auto">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    All order, buyer, and delivery info.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm">
                                    ...
                                    <div className="font-medium text-gray-600">
                                        Buyer Contact:{" "}
                                        <span className="font-normal text-gray-900">
                                            {selectedOrder.user.whatsapp_number}
                                        </span>
                                    </div>
                                    <div className="font-medium text-gray-600">
                                        Email:{" "}
                                ...
                                <div className="border rounded-lg p-4 bg-gray-50">
                                    ...
                                    <div className="grid grid-cols-3 gap-4 text-sm">
                                        ...
                                        <div className="font-semibold text-blue-600">
                                            AED{" "}
                                            {selectedOrder.totalCustomerPayment.toFixed(
                                                2
                                            )}
                                        </div>
                                        ...
                                        <div className="font-semibold text-gray-700">
                                            AED{" "}
                                            {selectedOrder.totalVendorPayment.toFixed(
                                                2
                                            )}
                                        </div>
                                        ...
                                        <div
                                            className={`font-semibold ${
                                                selectedOrder.totalProfit >= 0
                                                    ? "text-green-600"
                                                    : "text-red-600"
                                            }`}>
                                            AED{" "}
                                            {selectedOrder.totalProfit.toFixed(
                                                2
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div key={vehicle.id} className="mt-4">
                                        ...
                                        <ul className="list-none ml-0 text-sm text-gray-800 divide-y divide-gray-100">
                                            {vehicle.parts.map((part) => {
                                                ...
                                                const profit = part.accepted_bid
                                                    ? (part.accepted_bid
                                                          .customer_paid -
                                                          part.accepted_bid
                                                              .price) *
                                                ...
                                                return (
                                                    <li
                                                        ...
                                                        <div>
                                                            ...
                                                            {part.accepted_bid && (
                                                                <div className="text-xs text-gray-600 mt-1">
                                                                    Accepted:
                                                                    AED{" "}
                                                                    {part.accepted_bid.price.toFixed(
                                                                        2
                                                                    )}{" "}
                                                                    by{" "}
                                                                    {part
                                                        ...
                                                        {/* PART FINANCIAL DETAILS */}
                                                        {part.accepted_bid && (
                                                            <div className="mt-2 ml-2 p-2 bg-gray-50 rounded-md border text-xs">
            ...
            {selectedRefundRequest && (
                <Dialog
                    open={!!selectedRefundRequest}
                    onOpenChange={() => setSelectedRefundRequest(null)}>
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <p>
                                        {selectedRefundRequest.user.full_name}
                                    </p>
                                    <p className="text-gray-500">
                                        {selectedRefundRequest.user.email}
                                    </p>
                                    <p className="text-gray-500">
                                        {
                                            selectedRefundRequest.user
                                                .whatsapp_number
                                        }
                                    </p>
                                </div>
                                <div>
                                    ...
                                    <p>
                                        {selectedRefundRequest.part.part_name}
                                    </p>
                                    <p className="text-gray-500">
                                        {selectedRefundRequest.part.part_number}
                                    </p>
                                    <p className="text-gray-500">
                                        Quantity:{" "}
                                        {selectedRefundRequest.part.quantity}
                                    </p>
                                </div>
                                <div>
                                    <Label className="font-medium">
                                    ...
                                    <p>
                                        {
                                            selectedRefundRequest.part.vehicle
                                                .make
                                        }{" "}
                                        {
                                            selectedRefundRequest.part.vehicle
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-500">
                                        Date:{" "}
                                        {new Date(
                                            selectedRefundRequest.created_at
                                        ).toLocaleDateString()}
                                    </p>
                                </div>
                            </div>
                            ...
                            {selectedRefundRequest.images &&
                                selectedRefundRequest.images.length > 0 && (
                                    <div>
                                        <Label className="font-medium">
                                            Attached Images
                                        </Label>
                                        <div className="grid grid-cols-2 gap-2 mt-2">
                                            {selectedRefundRequest.images.map(
                                                (image, index) => (
                                                    <img
                                                        key={index}
                                                        src={
            ...
            <Dialog
                open={!!partToDelete}
                onOpenChange={() => !isDeleting && setPartToDelete(null)}>
                <DialogContent>
                    <DialogHeader>
                    ...
                    <div>
                        ...
                        {selectedOrder?.vehicles.flatMap((v) => v.parts)
                            .length === 1 && (
                            <span className="block mt-2 text-red-500">
                                Warning: This is the last part in the order -
                                deleting it will remove the entire order.
                            </span>
                        )}
                    </div>
                    <DialogFooter>
